...
xs = [0, 0, 1, -1, 1 * cos(pi/4), 1 * cos(pi/4), -1 * cos(pi/4), -1 * cos(pi/4)]
ys = [0.8, -0.8, 0, 0, 1 * sin(pi/4), -1 * sin(pi/4), 1 * sin(pi/4), -1 * sin(pi/4)]

plt.plot(xs, ys, linestyle="None", marker="o")
plt.show()

ellipse = Ellipse(1, 0.5, 0)
xs, ys = ellipse.samples()
points = list(zip(xs, ys))
selected = random.choices(points, k=8)
print()
...
def fit_ellipse(x, y):
    ...
    D2 = np.vstack([np.ones(len(x))]).T
    # D2 = np.vstack([np.ones(len(x)), np.ones(len(x)), np.ones(len(x))]).T
    S1 = D1.T @ D1
    S2 = D1.T @ D2
    S3 = D2.T @ D2
...
if __name__ == '__main__':
    ...
    x = [0, 0, 1, -1, 1 * cos(pi / 4), 1 * cos(pi / 4), -1 * cos(pi / 4), -1 * cos(pi / 4)]
    y = [0.8, -0.8, 0, 0, 1 * sin(pi / 4), -1 * sin(pi / 4), 1 * sin(pi / 4), -1 * sin(pi / 4)]
    noise = 0.1
    ...
    ellipse = Ellipse.from_points_using_gd(list(zip(x, y)))
    x, y = ellipse.samples()
