...
def stokes_basis_cam(pixel_size_H=2.74e-6, pixel_size_W=2.74e-6, pixel_H=512 * 2, pixel_W=612 * 2, focal=35e-3):
    """
    ...
    y, x = torch.meshgrid(torch.arange(min_y, max_y, pixel_size_H), torch.arange(min_x, max_x, pixel_size_W),
                          indexing='ij')
    R, C = x.shape
...
def rotate_stokes_vector(M_rot, stoke_current):
    ...
    stoke_target = np.einsum('ijk,ik->ij', M_rot, stoke_current.transpose(1, 0)).transpose(1,
                                                                                           0)  # [c=4, pixel_H x pixel_W]
...
def tgt2meas(stokes_global, poses, cam_kwargs={}):
    ...
    """
    assert stokes_global.ndim == 3 and poses.ndim == 2, f"dimensions of input stoke and pose are not 3 and 2, but {stokes_global.ndim} and {poses.ndim}"
